  ...
  switch (result) {
    ...
    case 'w:1':
      if (choice === 'w:1') {
        ...
        return 0;
      } else if (choice === 'w:2'|| choice === 'w+2:2') {
        return -1;
      }
      break;
    case 'd':
      if (choice === 'd') {
        return 2;
      } else if (choice === 'w:1' || choice === 'w:2') {
        return 0;
      } else if (choice === 'w+2:1' || choice === 'w+2:2') {
        return -1;
      }
      break;
    case 'w:2':
      if (choice === 'w:2') {
        ...
        return 0;
      } else if (choice === 'w:1' || choice === 'w+2:1') {
        return -1;
      }
      break;
    case 'w+2:2':
      ...
      Pick.find( { match: matchId }, function(err, picks) {
        picks.forEach(function (pick, index, array) {
          ...
          if (typeof newPoints !== 'undefined') {
            ...
            // update User.points w/ new value
            User.update({ _id: pick.user }, { $inc: { points: newPoints } }).exec();
          }
        });
      });
    }
